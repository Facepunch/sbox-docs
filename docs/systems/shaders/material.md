...
It does nothing on it's own but it's a contract on how the material would react to lighting, [Shading Models](shading-model.md) consume what comes from it.
...
* `Material::From( PixelInput i )` can be used to process the [standard input](reference/default-vertex-and-pixel-shader-inputs.md) and expose textures to be used directly to the Material Editor ( MET )
* To initialize an empty material you can use `Material::Init()` and fill up the needed fields for the [Shading Model](shading-model.md)
