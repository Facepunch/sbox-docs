...
struct Light
{
    ...
    // Gets the light at the given index — handles both dynamic (cluster-binned)
    // and static (probe / lightmap) lights transparently.
    static Light From( float3 vPositionWs, float4 vPositionSs, uint nLightIndex, float2 vLightMapUV = 0.0f );

    // Number of lights affecting the current fragment (dynamic + static).
...
for ( uint i = 0; i < Light::Count( ScreenPosition ); i++ )
{
    ...
    // ... use l.Color, l.Direction, l.Attenuation, l.Visibility ...
}
```

If you're using lightmaps and want the static-light pass to read from them, pass the lightmap UV as the optional fourth argument:

```cpp
Light l = Light::From( WorldPosition, ScreenPosition, i, lightMapUV );
```
